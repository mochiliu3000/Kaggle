...
def label_score(pred_temp_mean):
	pred_temp_label = np.round(pred_temp_mean)	
	#TODO: how to select threshold
	#0's: 549 -> 61.61%	 1's: 342 -> 38.39%  in training dataset
	threshold_ind = int(len(pred_temp_mean) * 0.6161)
	threshold = sorted(pred_temp_mean)[threshold_ind]
	#print("Threshold: %.6f" % threshold)
	pred_temp_label[pred_temp_mean > threshold] = 1
	return pred_temp_label
...
def precision(rater_a, rater_b):
	correct = len([i for i in range(len(rater_a)) if rater_a[i] == rater_b[i]])
	#print("Correct: %d, Overall: %d" % (correct, len(rater_a)))
	return correct / float(len(rater_a))
