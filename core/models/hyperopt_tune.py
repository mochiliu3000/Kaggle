...
def model_hyperopt_tune(config, feat_name, model_name, loss_fct):
	...
	print("Final Best: %.6f" % prec_cv_new)
	# log result, persist best and return best
	with open(base_path + "/" + model_name + "/best_params", "wb") as f:
		cPickle.dump(best, f, -1)
	return best
...
def model_valid(config, feat_name, model_name, params, loss_fct, n_run, n_fold, n_bag):
	...
	print("==================================================================")
	with open(out_folder + "/pred_cv_mean", "wb") as f:
		cPickle.dump(pred_cv_mean, f, -1)
	with open(out_folder + "/pred_cv_std", "wb") as f:
...
def model_test(config, feat_name, model_name, params, loss_fct, n_bag):
	...
	pred_out = pd.DataFrame({"id": id_test, "prediction": pred_raw})
	pred_out.to_csv("%s/%s" % (pred_raw_path, "pred.csv"), index=False)
